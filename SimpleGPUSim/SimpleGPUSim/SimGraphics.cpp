...
void SimGraphics::Initialize(const XMUINT2& renderTargetSize, uint32_t maxFacesPerIteration, const Camera& camera, uint32_t* frameBuffer)
{
    m_IsReady = false;
...
void SimGraphics::Clear()
{
    ...
    Color black(0.f, 0.f, 0.f, 1.0f);

    uint32_t totalLen = m_RenderTargetSize.x * m_RenderTargetSize.y;
    for (uint32_t i = 0; i < totalLen; i++)
    {
        m_Framebuffer[i] = (uint32_t)black;
    }
...
void SimGraphics::ComputeRaster(Face& face)
{
    ...
    if (maxX < 0.0f || minX >(m_RenderTargetSize.x - 1) || maxY < 0.0f || minY >(m_RenderTargetSize.y - 1))
    {
        face.m_Hidden = true;
    ...
    for (uint32_t x = face.m_BoundingBox.m_MinX; x <= face.m_BoundingBox.m_MaxX; ++x)
    {
        for (uint32_t y = face.m_BoundingBox.m_MinY; y <= face.m_BoundingBox.m_MaxY; ++y)
        {
               ...
               (p2 = EdgeFunction(face.m_Vertex[0].m_Position, face.m_Vertex[1].m_Position, pixelPoint)) >= 0)
            {
                ...
                uint32_t arrPos = (y * m_RenderTargetSize.x) + x;
                ...
                if (isVisible)
                {
                    ...
                    m_Framebuffer[arrPos] = pixelColor;
                }
            }
        }
    }
